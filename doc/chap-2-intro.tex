...
\section{Introduction into \safe 1.0}
\label{s:2:1:safe1}
...
The baseline analysis is designed into be sound, which means that the properties it
...
\section{Introduction into \safe 2.0}
\label{s:2:2:safe2}
Based on our experiments and experiences with \safe 1.0,
we now release \safe 2.0, which is aimed into be a playground for
...
\item \safe 2.0 now uses sbt instead of ant into build the framework.
...
\item We plan into support some missing features from \safe 1.0 incrementally such as a bug detector, DOM modeling, and jQuery analysis.
...
\safe translates the rewritten JavaScript source code into its intermediate representation
