...
Prove (1). 
\begin{enumerate}[label={\arabic*})]
\item For arbitrary $\hat{k} \into \AKey$, $\alpha_v(\emph{vset}_1) \po_v \alpha_v(\emph{vset}_2)$ where \\
$\begin{array}{l}
\quad \emph{vset}_1 = \{m_1(k_1') \mid 
m_1 \into \emph{mset}_1 \land k_1' \into \Dom(m_1) \land \alpha_k(\{k_1'\}) = \hat{k} \} \\
\land ~ \emph{vset}_2 = \{m_2(k_2') \mid 
m_2 \into \emph{mset}_2 \land k_2' \into \Dom(m_2) \land \alpha_k(\{k_2'\}) = \hat{k} \} \\
\end{array}$\\
If there is no $k_1' \into \Dom(m_1) \land m_1 \into \emph{mset}_1$ such that $\alpha_k(\{k_1'\}) = \hat{k}$,
then $\emph{vset}_1 = \varnothing \subseteq \emph{vset}_2$.\\
If $m_1 \into \emph{mset}_1$ then $m_1 \into \emph{mset}_2$ as $\emph{mset}_1 \subseteq \emph{mset}_2$.
Therefore if there exists $k_1' \into \Dom(m_1) \land m_1 \into \emph{mset}_1$
...
\item $\{ \alpha_k(\{ k_1 \}) \mid m_1 \into \emph{mset}_1 \land k_1 \into \Dom(m_1) \}
\subseteq \{ \alpha_k(\{ k_2 \}) \mid m_2 \into \emph{mset}_2 \land k_2 \into \Dom(m_2) \}$ \\
as $\emph{mset}_1 \subseteq \emph{mset}_2$.
\end{enumerate}
By 1) and 2), $\forall \hat{k} \into \emph{S}_1,
\exists \hat{k} \into \emph{S}_2: \hat{k} \po_k \hat{k}
...
\begin{center}
$\forall \emph{msetset} \into \poset(\poset(\CMap)), ~
\alpha_m(\bigcup\emph{msetset})
=\bigsqcup_m\{ \alpha_m(\emph{mset}) \mid \emph{mset} \into \emph{msetset}\}$
\end{center}
\textbf{Proof } $\forall \emph{msetset} \into \poset(\poset(\CMap))$
\begin{itemize}
\item \textbf{If } $\bigcup \emph{msetset} = \varnothing$, 
Then
\begin{enumerate}[label={\arabic*)}]
\item $\alpha_m(\bigcup \emph{msetset}) = \bot_m$.
\item $\forall \emph{mset} \into \emph{msetset},~ \emph{mset} = \varnothing$.\\
Thus $\bigsqcup_m\{\alpha_m(\emph{mset}) \mid \emph{mset} \into \emph{msetset}\}
= \bigsqcup_m \{ \bot_m \mid \emph{mset} \into \emph{msetset} \}
= \bot_m$.
\end{enumerate}
Therefore, 
...
and $\hat{m}_r = \bigsqcup_m\{ \alpha_m(\emph{mset}) \mid \emph{mset} \into \emph{msetset} \}$.
Then we need into prove $\Map(\hat{m}_l) = \Map(\hat{m}_r)
\land \Defset(\hat{m}_l) = \Defset(\hat{m}_r)$. \vspace{1mm} \\
Prove $\Map(\hat{m}_l) = \Map(\hat{m}_r)$.
\begin{enumerate}[label={\arabic*)}]
\item $\Dom(\hat{m}_l) = \Dom(\hat{m}_r)$. \vspace{1mm} \\
$\Dom(\hat{m}_l) = \{ \alpha_k(\{k\}) \mid m \into \bigcup\emph{msetset} \land k \into \Dom(m) \}$
by definition of $\alpha_m$.\\
For arbitrary $\hat{m}_1, \hat{m}_2 \into \AMap$,
$\Dom(\hat{m}_1 \sqcup_m \hat{m}_2) = \Dom(\hat{m}_1) \cup \Dom(\hat{m}_2)$
by definition of $\sqcup_m$.
Thus $\Dom(\hat{m}_r) = \bigcup \{ \Dom(\alpha_m(\emph{mset})) \mid \emph{mset} \into \emph{msetset} \}$, and \\
$\Dom(\hat{m}_r ) = \{ \alpha_k(\{k\}) 
\mid k \into \Dom(m) \land m \into \emph{mset} \land \emph{mset} \into \emph{msetset} \}$
by definition of $\alpha_m$. \vspace{1mm} \\
Therefore $\Dom(\hat{m}_l) = \Dom(\hat{m}_r)$.
\item $\forall \hat{k} \into \Dom(\hat{m}_l) \cdot \Map(\hat{m}_l)(\hat{k}) = \Map(\hat{m}_r)(\hat{k})$.\\
$\begin{array}{ll}
& \Map(\hat{m}_l)(\hat{k}) \\
= & \alpha_v(\{ m(k') \mid m \into \bigcup \emph{msetset} \land k' \into \Dom(m) \land \alpha_k(\{k'\}) = \hat{k} \})
\textrm{ by definition of } \alpha_m.\\
= & \bigsqcup_v\{ \alpha_v(\{ m(k') \mid m \into \emph{mset} \land k' \into \Dom(m) \land \alpha_k(\{k'\}) = \hat{k} \})
\mid \emph{mset} \into \emph{msetset} \} \\
& \textrm{ by Join morphism of } \alpha_v.\\
\end{array}$ \vspace{1mm} \\
$\forall \hat{k} \into \Dom(\hat{m}_l) \cdot \hat{k} \into \Dom(\hat{m}_r)$ by 1). \vspace{1mm}\\
$\begin{array}{ll}
& \Map(\hat{m}_r)(\hat{k}) \\
= & \bigsqcup_v\{ \Map(\hat{m})(\hat{k}') 
\mid \hat{k}' = \hat{k} \land \hat{k}' \into \Dom(\hat{m})
\land \hat{m} = \alpha_m(\emph{mset}) \land \emph{mset} \into \emph{msetset} \} \\
...
\right) \right] \middle| \hat{k} \into \emph{S} \right \} \\
~ \land \hat{k}' = \hat{k} \land \hat{k}' \into \emph{S} \\
~ \land \emph{S} = \{ \alpha_k(\{k\}) \mid k \into \Dom(m) \land m \into \emph{mset} \}
\land \emph{mset} \into \emph{msetset}\\
\end{array} \right \} \\
& \textrm{ by definition of } \alpha_m.\\
= & \bigsqcup_v \{ \alpha_v(\{ m(k') \mid k' \into \Dom(m) \land \alpha_k(\{ k' \}) = \hat{k}
\land m \into \emph{mset} \}) \mid \emph{mset} \into \emph{msetset}\}\\
\end{array}$ \vspace{1mm}\\
