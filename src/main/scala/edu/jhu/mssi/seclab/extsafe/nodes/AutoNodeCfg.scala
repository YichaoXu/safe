...
class AutoNodeCfg(
  ...
  private val nodesCsvPath: Path,
  private val edgesCsvPath: Path
)extends CFG(delegated.ir, delegated.globalVars){

  override val globalFunc:CFGFunction = delegated.globalFunc

  private val nodesSet = getSetFromCsv(nodesCsvPath)
  private val edgesSet = getSetFromCsv(edgesCsvPath)

  private def getSetFromCsv(filePath: Path): mutable.Map[String, Map[String, String]] = {
    val output = mutable.Map[String, Map[String, String]]()
    val nodesCsv = io.Source.fromFile(nodesCsvPath.toString)
  ...
  private def findNode(span: Span): Option[(String, Map[String, String])] = {
    nodesSet.find { case(key:String, value:Map[String, String]) =>
  ...
  override def createFunction(
    ...
    name: String, ir: IRNode, isUser: Boolean
  ): CFGFunction = if (isNodeExists(ir.span))
