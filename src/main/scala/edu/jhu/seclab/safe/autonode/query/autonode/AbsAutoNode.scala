...
abstract class AbsAutoNode {
  ...
  def fileEntry: Option[SignatureNode]
  ...
  def signatureOf(entry: ModelNode): Option[SignatureNode] = this.prev(of = entry, eType = ENTRY).headOption match {
  ...
  def entryOf(signature: SignatureNode): Option[ModelNode] = this.next(of = signature.core, eType = ENTRY).headOption
