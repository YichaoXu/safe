...
import edu.jhu.seclab.safe.autonode.query.csv.model.EdgeType.{EdgeType, FLOWS_TO}
...
class AutoNodeCsv(nodeFile: File, edgeFile: File) {
  ...
  def next(of: ModelNode, edgeType: EdgeType = FLOWS_TO): Seq[ModelNode] =
    edges.filter(edge => edge.is(edgeType) && edge.start == of.id).map{ edge => node(id=edge.end).get }

  def prev(of: ModelNode, edgeType: EdgeType = FLOWS_TO): Seq[ModelNode] =
    edges.filter(edge => edge.is(edgeType) && edge.end == of.id).map{ edge => node(id=edge.start).get }
