...
import edu.jhu.seclab.safe.autonode.cfg.block.NormalBlockHolder
...
class AutoNodeCfgHolder(
  ...
  private def newFunctionHolder(funcId: Int): Unit = AutoNodeCsv.query.node(id = funcId) match {
    case Some(funcDef) =>
      ...
      val entryBlock = new NormalBlockHolder().append(startNode)
      val newBlock = new NormalBlockHolder().flowFrom(entryBlock)
  ...
  private def newBlockHolder(from: Option[ModelNode], forFunc: String, curHolder: AbsBlockHolder): Unit = from match {
    case Some(nextStart) if curHolder.isClosed =>
      ...
      funcsMap(forFunc) += new NormalBlockHolder().flowFrom(curHolder)
      newBlockHolder(from, forFunc, funcsMap(forFunc).last.get)
    ...
    case Some(funcExit) if funcExit isFuncEnd =>
      curHolder.close()
      funcsMap(forFunc) += new NormalBlockHolder().append(funcExit).flowFrom(curHolder).close()
