...
case class BoolOption[PhaseConfig <: Config](
  assign: PhaseConfig => Unit) extends OptionKind[PhaseConfig] {
...
case class NumOption[PhaseConfig <: Config](
  assign: (PhaseConfig, Int) => Unit) extends OptionKind[PhaseConfig] {
  def argRegexList(name: String): List[ArgRegex[PhaseConfig]] = List(
...
case class StrOption[PhaseConfig <: Config](
  assign: (PhaseConfig, String) => Unit) extends OptionKind[PhaseConfig] {
  def argRegexList(name: String): List[ArgRegex[PhaseConfig]] = List(
...
case class ListOption[PhaseConfig <: Config](
  assign: (PhaseConfig, List[String]) => Unit) extends OptionKind[PhaseConfig] {
  def argRegexList(name: String): List[ArgRegex[PhaseConfig]] = List(
